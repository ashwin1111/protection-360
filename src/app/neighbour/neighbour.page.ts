...
public profile_url: string;
  ...
  addpost() {
    let dref = firebase.database().ref('question');
    dref.once('value', res => {
...
<i class="mdi mdi-format-quote-open font-20"></i>` + snapshot.val().question + `</div></div><br>`;
          if (snapshot.hasChild('reply')) {
                  ...
                  let ref1 = firebase.database().ref('question/' + snapshot.key + '/reply');
                  ref1.once('value', res1 => {
                 let inner = `<div id="` + snapshot.key + `comment" class="post-user-comment-box">`;
...
let inner1 = `<div id="` + snapshot.key + `comment" class="post-user-comment-box">`;
...
reply(key) {
  let info = {
    ...
    name: this.user_name,
    reply: (document.getElementById(key + 'simpleinput') as HTMLInputElement).value
  };
